...
- コードベースの調査が必要な場合は **OpenCode の `explorer` エージェント**に委譲する（Delegation rules 参照）
  - ⚠️ ClaudeCode native の `Explore` agent type（Agent tool）は**使わない**。必ず `/ask-opencode --agent explorer` 経由で呼ぶ。
...
コード調査が必要な場合は必ず **OpenCode の `explorer` エージェント**（`opencode run --agent explorer`）に委譲する。

⚠️ **ClaudeCode native の `Explore` agent type は使わない。** Agent tool で `subagent_type: "Explore"` を指定するのは禁止。
調査委譲は必ず `/ask-opencode --agent explorer` 経由。
